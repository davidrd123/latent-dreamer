...
Lane 2: Deep Build (kernel rule engine)            ← where the architecture lives
  ...
  ├── Next Graffito reusable seams: broader use-outcome tracking, the generic post-effect hook now in kernel with Graffito reread projection still benchmark-local, and the next non-rehearsal family ecology move chosen from longer-run evidence
