...
(deftest roving-plan-effects-builds-typed-program
  (let [[world root-id] (world-with-root)
    ...
    (is (= (str "roving:pleasant_episode_seed:"
                pleasant-episode-id)
...
(deftest evaluator-promotion-decision-does-not-open-frontier-rules-at-store-time
  (let [promote-evaluator (fn [_family-plan default-evaluation]
                            (assoc default-evaluation
    ...
    (is (= :frontier
           (get-in world [:rule-access :goal-family/reversal-aftershock-to-rationalization :status])))
    (is (= :authored-frontier
           (get-in world [:rule-access :goal-family/reversal-aftershock-to-rationalization :source])))
...
(deftest hard-failure-family-plan-admission-does-not-open-frontier-rules-at-store-time
  (let [contradicted-evaluator (fn [_family-plan default-evaluation]
                                 (assoc default-evaluation
                                        :promotion-decision :promote-durable
        ...
        [world family-plan] (families/run-family-plan world
                                                      {:goal-id roving-goal-id
                                                       :context-id context-id
                                                       :family-evaluator contradicted-evaluator})
        family-episode-id (:family-episode-id family-plan)]
    (is (= :promote-durable (get-in family-plan [:evaluation :promotion-decision])))
    (is (= [:contradicted] (get-in family-plan [:evaluation :anti-residue-flags])))
    (is (= :provisional (:admission-status family-plan)))
    (is (= :provisional
           (get-in world [:episodes family-episode-id :admission-status])))
    (is (= :frontier
           (get-in world [:rule-access :goal-family/reversal-aftershock-to-rationalization :status])))
    (is (= :authored-frontier
           (get-in world [:rule-access :goal-family/reversal-aftershock-to-rationalization :source])))
  ...
  (let [[world root-id] (world-with-root)
        ...
        (fn [_family-plan _default-evaluation]
          {:realism :plausible
           ...
           :promotion-decision :stay-provisional
           :anti-residue-flags [:contradicted]
           :evaluation-reasons [:mock_contradicted_source]
           :evaluation-source :mock-llm})
        world (-> world
                  (cx/assert-fact trigger-context-id guide-fact)
                  (cx/assert-fact trigger-context-id {:fact/type :goal
    ...
    (is (= [:contradicted]
           (mapv :outcome
                 (get-in later-family-plan
                         [:result :episode-outcome-facts]))))
    (is (= :provisional (:admission-status stored-episode)))
    (is (= [:contradicted] (:anti-residue-flags stored-episode)))
    (is (= 1 (get-in stored-episode [:outcome-stats :contradiction-count])))
    (is (= [{:from-status :durable
...
(deftest reused-reversal-source-episode-can-backfire-and-demote
  (let [[world root-id] (world-with-root)
        ...
        backfire-evaluator
        (fn [_family-plan _default-evaluation]
          {:realism :counterfactual
           :desirability :negative
           :retention-class :payload-exemplar
           :keep-decision :keep-exemplar
           :promotion-decision :stay-provisional
           :anti-residue-flags [:backfired]
           :evaluation-reasons [:mock_backfired_source]
           :evaluation-source :mock-llm})
        world (cx/assert-fact world old-context-id explicit-cause)
        [world initial-reversal-goal-id]
        (goals/activate-top-level-goal
    ...
    (is (= [:backfired]
           (mapv :outcome
                 (get-in later-family-plan
                         [:result :episode-outcome-facts]))))
    (is (= :provisional (:admission-status stored-episode)))
    (is (= [:backfired] (:anti-residue-flags stored-episode)))
    (is (= 1 (get-in stored-episode [:outcome-stats :backfire-count])))
    (is (= [{:from-status :durable
             :to-status :provisional
             :cycle 0
             :reason :outcome-driven-demotion}]
...
(deftest frontier-activation-rule-does-not-dispatch-family-goal
  (let [activation-rules* (->> (families/activation-rules)
                               (mapv (fn [rule]
                                       (if (= :goal-family/roving-activation
                                              (:id rule))
                                         (assoc-in rule
                                                   [:provenance :deployment-role]
                                                   :authored-frontier)
                                         rule))))
        family-rules* (vec (concat (families/planning-rules)
                                   (families/cross-family-rules)
                                   activation-rules*))
        [world root-id] (world-with-root)
        world (assoc world :reality-context root-id)
        ...
        world (-> world
                  ...
                  (cx/assert-fact context-id {:fact/type :dependency
                                              :from-id emotion-id
                                              :to-id failed-goal-id}))
        world (with-redefs [families/activation-rules (constantly activation-rules*)
                            families/family-rules (constantly family-rules*)]
                (families/activate-family-goals world))]
    (is (empty? (filter #(= :roving (:goal-type %))
                        (vals (:goals world)))))
    (is (= :frontier
           (get-in world [:rule-access :goal-family/roving-activation :status])))))
...
(deftest rationalization-plan-effects-rejects-missing-producer-wiring
  (let [[_world root-id] (world-with-root)
        validator @#'daydreamer.goal-families/validate-family-effect-program!]
    (is (thrown-with-msg? clojure.lang.ExceptionInfo
                          #"missing prior result-key"
                          (validator
                           {:rule {:id :goal-family/rationalization-plan-dispatch}
                            :result {:effects [{:op :context/sprout
                                               ...
                                               {:op :rationalization/assert-afterglow
                                                ...
                                                :failed-goal-id :g-failed
                                                :frame-id :rf-1
                                                :fallback-trigger-emotion-id :e-1
                                                :fallback-trigger-emotion-strength 0.5
                                                :rule-provenance {:rule-path []
                                                                  :edge-path []}
                                                :from-diversion :rationalization/diversion
...
(deftest reversal-plan-effects-rejects-invalid-branch-payload
  (let [[world root-id] (world-with-root)
        [world old-context-id old-top-level-goal-id emotion-id]
        (seed-reversal-bridge-context world root-id)
        [world reversal-goal-id]
        ...
        reversal-target {:goal-id reversal-goal-id
                         ...
                         :new-top-level-goal-id reversal-goal-id
                         :input-facts [counterfactual-fact]}]
    (with-redefs [daydreamer.goal-families/reverse-leaf-branches
                  (constantly [{:old-context-id old-context-id
                                :old-top-level-goal-id old-top-level-goal-id
                                :leaf-strength 0.1
                                :ordering 10.0
                                :objective-facts []
                                :selection-policy gf-rules/reverse-leaf-policy
                                :selection-reasons [:intends_leaf]}])]
      (is (thrown-with-msg? clojure.lang.ExceptionInfo
                            #"Invalid reversal branch payload"
                            (families/reversal-plan-effects
                             world
                             reversal-target))))))
